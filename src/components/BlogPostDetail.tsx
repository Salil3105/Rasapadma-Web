...
import { ArrowLeft, Calendar, User, Clock, Heart, Eye, Share2 } from 'lucide-react';
      ...
      <p key={i} className="mb-4 text-slate-600 dark:text-slate-300 leading-relaxed">
        {parts.map((part, j) =>
          part.startsWith('**') && part.endsWith('**') ? (
            <strong key={j} className="text-slate-800 dark:text-slate-200 font-semibold">
              {part.slice(2, -2)}
            </strong>
          ) : (
            part
...
function copyToClipboard(text: string) {
  navigator.clipboard.writeText(text).then(() => {
    if (typeof document !== 'undefined') {
      ...
      toast.className = 'fixed bottom-4 right-4 px-4 py-2 bg-slate-800 text-white rounded-lg text-sm z-50';
...
export const BlogPostDetail = ({ post, onBack }: BlogPostDetailProps) => {
  const { isLiked, toggleLike, getLikeCount, getViewCount, incrementView } = useBlog();
